...
class BarChartData extends AxisChartData with EquatableMixin {
    ...
    Color? backgroundColor,
      ...
      : barGroups = barGroups ?? const [],
        ...
        super(
          ...
          minX: 0,
          maxX: 1,
          maxY: maxY ?? calculateMaxY(barGroups ?? []),
          minY: minY ?? calculateMinY(barGroups ?? [])
      );

  static double calculateMaxY(List<BarChartGroupData> barGroups) {
  ...
  static double calculateMinY(List<BarChartGroupData> barGroups) {
    for (var i = 0; i < barGroups.length; i++) {
      final barData = barGroups[i];
      if (barData.barRods.isEmpty) {
        throw Exception('barRods could not be null or empty');
